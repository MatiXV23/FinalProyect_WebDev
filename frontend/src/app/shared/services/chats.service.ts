...
export class ChatsService {
  ...
  public async getAll(): Promise<Chat[]> {
    ...
    return await firstValueFrom(
      this.httpClient.get<Chat[]>(`${environment.apiUrl}/usuarios/${id_usuario}/chats`)
    );
  }
  ...
  public async getChatById(id_chat: string): Promise<Chat> {
    ...
    return await firstValueFrom(
      this.httpClient.get<Chat>(`${environment.apiUrl}/usuarios/${id_usuario}/chats/${id_chat}`)
    );
  }
  ...
  public async getMensajes(id_chat: string): Promise<Mensaje[]> {
    ...
    return await firstValueFrom(
      this.httpClient.get<Mensaje[]>(
        `${environment.apiUrl}/usuarios/${id_usuario}/chats/${id_chat}/mensajes`
      )
    );
  }
  ...
  public async getChatId(id_comprador: number, id_vendedor: number): Promise<number> {
    ...
    const chat = chats.find(
      (c) =>
        (c.id_comprador === id_comprador && c.id_vendedor === id_vendedor) ||
        (c.id_comprador === id_vendedor && c.id_vendedor === id_comprador)
    );

    if (chat) return chat.id_chat;

    const nuevo_chat = await firstValueFrom(
      this.httpClient.post<Chat>(`${environment.apiUrl}/usuarios/${id_usuario}/chats`, {
        id_comprador,
        id_vendedor,
      })
    );
