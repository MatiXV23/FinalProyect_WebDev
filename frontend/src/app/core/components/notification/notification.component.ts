...
export class NotificationComponent implements OnInit {
  ...
  getNotificationColors(type: 'error' | 'success' | 'info' | 'warning'): string[] {
    switch (type) {
      case 'error':
        return ['text-red-600', 'text-red-800', 'hover:text-red-900', 'border-red-500'];
      case 'success':
        return ['text-green-600', 'text-green-800', 'hover:text-green-900', 'border-green-500'];
      case 'info':
        return ['text-blue-600', 'text-blue-800', 'hover:text-blue-900', 'border-blue-500'];
      default:
        return ['text-yellow-600', 'text-yellow-800', 'hover:text-yellow-900', 'border-yellow-500'];
    }
  }
