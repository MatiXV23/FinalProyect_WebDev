...
export class ChatsDB {
    ...
    async getAll(id_usuario: number): Promise<Chat[]> {
        try {
            const chats = await this.pool.query<Chat>(this.getQuery('WHERE c.id_vendedor = $1 OR c.id_comprador = $1'), [id_usuario])
            return chats.rows
    ...
    async getById(id_chat: number): Promise<Chat> {
        try {
            const chats = await this.pool.query<Chat>(this.getQuery('WHERE c.id_chat = $1'), [id_chat])

            if (chats.rowCount === 0) throw new PC_NotFound(`Chat con id (${id_chat}) no encontrado`)
            return chats.rows[0]
        } catch (e) {
            throw new PC_InternalServerError()
        }
    ...
    async create(data: Partial<Chat>): Promise<Chat> {
        ...
        try{
            const res = await this.pool.query<Chat>(query, [id_comprador, id_vendedor])
            return res.rows[0]
        }catch(err: any){
            throw new PC_InternalServerError()
        }
    ...
    async delete(id_chat: number): Promise<void> {
        try {
           const query =  `DELETE FROM usuarios
            ...
            if (res.rowCount === 0) throw new PC_NotFound(`Usuario de id ${id_chat}, no existe. Ignorando`)
            console.log(res) 
        } catch(e) {
            throw new PC_InternalServerError()
        }
    ...
    async getMensajesFromChat(id_chat: number): Promise<Mensaje[]> {
        try {
            const chats = await this.pool.query<Mensaje>(this.getMsgQuery('WHERE m.id_chat = $1'), [id_chat])
            return chats.rows
        } catch (e) {
            throw new PC_InternalServerError()
        }
    ...
    async createMensajeForChat(id_chat: number, data: Partial<Mensaje>): Promise<Mensaje> {
        ...
        let query = /*sql*/` 
                    ...
                    SELECT * from nuevo_chat;`
        try{
            const res = await this.pool.query<Mensaje>(query, [id_chat, id_enviador, contenido])
            return res.rows[0]
        }catch(err: any){
            throw new PC_InternalServerError()
        }
