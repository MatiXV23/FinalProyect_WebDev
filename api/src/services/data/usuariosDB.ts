...
export class UsuariosDB extends BasePgRepository<Usuario> {
    ...
    async create(data: UsuarioPost): Promise<Usuario> {
        const {is_admin, email, nombres, apellidos, id_departamento, direccion,  nro_documento, password ,foto_url = 'https://img.freepik.com/vector-premium/icono-perfil-avatar_188544-4755.jpg?w=1060'} = data
        ...
        try{
            const res = await this.pool.query(query, [is_admin, email, nombres, apellidos, id_departamento, direccion,  nro_documento, foto_url, password])
            return res.rows[0]
        }catch(err: any){
    ...
    async update(id: number, data: Partial<Usuario>): Promise<Usuario> {
        ...
        for (const key in data){
            ...
            if (data[k]===undefined || k === 'id_usuario' || k === 'reputacion') continue
    ...
    async getUserByCredentials(credenciales: Credenciales): Promise<Usuario> {
        const query = this.getQuery(`JOIN credenciales c ON c.id_usuario = u.id_usuario
